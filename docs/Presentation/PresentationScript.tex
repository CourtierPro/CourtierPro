...
\begin{itemize}
    ...
    \item 13 Fully Implemented | 10 Modified | 4 Not Implemented
...
Of the 26 original use cases, 13 were fully implemented as proposed, 10 were modified to better fit real-world Quebec real estate workflows, and 4 were intentionally not implemented --- with the client's agreement. On top of that, we added over 50 new use cases during our five development sprints.
\end{speakernote}
...
\begin{itemize}
    \item Broker views dashboard with KPIs, priority cards, activity feed
    \item Broker filters transactions (side, status, search)
    \item Broker updates transaction stage with notifications and timeline logging
    \item Broker receives real-time in-app + email notifications (SES)
    \item Broker views transaction timeline (audit trail)
    \item Broker views tabbed transaction details (9 tabs)
    \item Client logs in via Auth0 with role-based routing
    \item Client views transaction progress via stage tracker
    \item Admin views login, password reset, and system audit logs
    \item Admin manages language settings (organization and user level)
    \item Admin edits email templates (11 types, EN/FR, live preview)
    \item Admin manages user accounts (invite, activate, deactivate, reset password)
    \item User logs in and resets password via Auth0
\end{itemize}
\end{slidedesc}
...
\begin{itemize}
    \item Prioritize transactions $\rightarrow$ Pin system + smart priority cards
    \item Assign transactions $\rightarrow$ Co-managing brokers via Participants tab
    \item Export transaction summary $\rightarrow$ Analytics module with CSV and PDF export
    \item Enter milestone dates $\rightarrow$ Automatic timeline logging per stage
    \item Client transaction summary $\rightarrow$ Full dashboard with KPIs, carousel, widgets
\end{itemize}
\end{slidedesc}
...
\begin{itemize}
    \item Progress percentage $\rightarrow$ Visual stage tracker for buyer/seller workflows
    \item Milestone confirmation $\rightarrow$ Document review workflow (approve/revision/reject)
    \item Configurable stages $\rightarrow$ Fixed industry-standard QC stages
    \item Archive transactions $\rightarrow$ Soft-delete and restore with audit trail
    \item Custom permissions $\rightarrow$ Fixed role-based system (Broker/Client/Admin)
\end{itemize}
\end{slidedesc}

\begin{speakernote}
Ten use cases were modified. In every case, the modification was driven by practical needs. For example, instead of abstract percentage-based progress, we built a visual stage tracker that maps directly to Quebec real estate stages, like ``Financial Preparation'' through ``Possession'' for buyers. Instead of assigning transactions between brokers --- which doesn't match a small brokerage's workflow --- we added co-managing brokers through the Participants tab.
...
\begin{itemize}
    \item Tax breakdown (GST/QST) --- Handled by notaries at closing
    \item Commission tax calculation --- Falls to brokerage accounting
    \item Commission amount input --- Related to tax features above
    \item Dashboard configuration --- Standardized dashboards chosen for consistency
    \item System tax settings --- Same rationale as tax breakdown
\end{itemize}
